...
import Loader from '../components/common/Loader';
import useFetchData from '../hooks/useFetchData';
import { Files, User, ShieldX, BrainCircuit, Users, StickyNote, HelpCircle, Maximize2, Minimize2, Filter, ArrowRight, ArrowLeft } from 'lucide-react';
...
const NetworkGraph = ({ personId }) => {
    ...
    const applyFilters = useCallback(() => {
        ...
        // Filter edges based on all criteria
        const filteredEdges = graphData.edges.filter(edge => {
            const amount = parseInt(edge.label.replace(/[^0-9]/g, ''));
        ...
        // Determine which nodes are still visible
        ...
        const filteredNodes = graphData.nodes.filter(node => 
            visibleNodeIds.has(node.id) && visibleTypes.includes(node.type)
        );

        // --- NEW: Circular Layout Algorithm ---
        const peerNodes = filteredNodes.filter(n => !n.isCenter);
        const formattedNodes = filteredNodes.map((node, index) => {
            const angle = (index / (peerNodes.length || 1)) * 2 * Math.PI;
            const radius = node.isCenter ? 0 : 350;
            return {
                id: node.id,
                position: { 
                ...
                style: {
                    background: node.type === 'Person' ? 'linear-gradient(135deg, #0891b2 0%, #06b6d4 100%)' : 'linear-gradient(135deg, #be185d 0%, #db2777 100%)',
                    color: 'white',
                    border: node.isCenter ? '4px solid #facc15' : '2px solid rgba(255,255,255,0.3)',
                    borderRadius: '50%',
                    width: node.isCenter ? 120 : 80,
                    height: node.isCenter ? 120 : 80,
                    fontSize: '12px',
                    ...
                    justifyContent: 'center',
                    padding: '5px',
                    boxShadow: node.isCenter ? '0 0 40px rgba(250, 204, 21, 0.5)' : '0 10px 20px rgba(0,0,0,0.4)',
                    animation: node.isCenter ? 'pulse 2.5s infinite' : 'none',
        ...
        setNodes(formattedNodes);
        setEdges(filteredEdges.map((edge, i) => ({
            id: `e${i}`,
            source: edge.isOutgoing ? edge.source : edge.target,
            target: edge.isOutgoing ? edge.target : edge.source,
            label: edge.label,
            type: 'smoothstep',
            markerEnd: { type: MarkerType.ArrowClosed, color: '#0891b2', width: 20, height: 20 },
            style: { stroke: '#0891b2', strokeWidth: 2.5 },
            labelStyle: { fill: 'white', fontWeight: 'bold' },
            labelBgStyle: { fill: '#1f2937', padding: '3px 5px', borderRadius: '4px' },
        })));
    ...
    if (error) return <p className="text-red-400 p-4">Error loading graph: {error.message}</p>;
    ...
    return (
        <div className="h-full w-full relative">
            ...
            <div className="absolute top-4 left-4 z-10 bg-gray-800/90 backdrop-blur-sm rounded-lg p-4 border border-gray-600 w-64 animate-fadeIn">
                <h4 className="text-sm font-semibold mb-3 text-gray-200 flex items-center"><Filter className="h-4 w-4 mr-2"/>Graph Filters</h4>
                <div className="space-y-4">
                    <div className="text-xs">
                        <label className="block mb-1 text-gray-400">Min. Transaction (₹{filterAmount.toLocaleString('en-IN')})</label>
                        <input type="range" min="0" max="100000" step="5000" value={filterAmount} onChange={(e) => setFilterAmount(Number(e.target.value))} className="w-full h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer" />
                    </div>
                    <div className="text-xs">
                        <label className="block mb-2 text-gray-400">Transaction Direction</label>
                        <div className="flex bg-gray-900/50 p-1 rounded-md">
                            <FilterButton active={direction === 'all'} onClick={() => setDirection('all')}>All</FilterButton>
                            <FilterButton active={direction === 'outgoing'} onClick={() => setDirection('outgoing')}><ArrowRight className="h-3 w-3 mr-1"/>Out</FilterButton>
                            <FilterButton active={direction === 'incoming'} onClick={() => setDirection('incoming')}><ArrowLeft className="h-3 w-3 mr-1"/>In</FilterButton>
                        </div>
                    </div>
                     <div className="flex space-x-4 text-xs">
                        <label className="flex items-center space-x-2 cursor-pointer">
                            <input type="checkbox" checked={visibleTypes.includes('Person')} onChange={() => setVisibleTypes(p => p.includes('Person') ? p.filter(t => t !== 'Person') : [...p, 'Person'])} className="form-checkbox h-4 w-4 bg-gray-700 border-gray-600 text-cyan-600 focus:ring-cyan-500 rounded"/>
                            <span className='text-gray-300'>Persons</span>
                        </label>
                        <label className="flex items-center space-x-2 cursor-pointer">
                            <input type="checkbox" checked={visibleTypes.includes('Company')} onChange={() => setVisibleTypes(p => p.includes('Company') ? p.filter(t => t !== 'Company') : [...p, 'Company'])} className="form-checkbox h-4 w-4 bg-gray-700 border-gray-600 text-pink-600 focus:ring-pink-500 rounded"/>
            ...
            {selectedNode && (
                <div className="absolute top-4 right-4 z-10 bg-gray-800/90 backdrop-blur-sm rounded-lg p-4 border border-gray-600 w-64 animate-fadeIn">
                    ...
                    <button onClick={() => setSelectedNode(null)} className="mt-3 text-xs w-full bg-gray-700 hover:bg-gray-600 px-3 py-1 rounded text-white transition">Close</button>
                </div>
            )}
            ...
            <ReactFlow nodes={nodes} edges={edges} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} onNodeClick={onNodeClick} fitView>
                <MiniMap style={{ backgroundColor: '#111827' }} nodeColor={n => n.style.background} />
                <Controls />
...
const FilterButton = ({ active, children, ...props }) => (
    <button {...props} className={`flex-1 flex items-center justify-center text-center px-2 py-1 rounded-sm text-xs transition ${active ? 'bg-cyan-600 text-white font-bold' : 'hover:bg-gray-700 text-gray-300'}`}>
        {children}
    </button>
);

// --- Main Workspace Component with Fullscreen Toggle ---
const InvestigationWorkspace = ({ /* Props are passed down */ }) => {
    const { caseId } = useParams();
    const [isGraphFullscreen, setIsGraphFullscreen] = useState(false);
                ...
                <div className="flex-1 flex flex-col overflow-hidden">
                    ...
                    <main className="flex-1 flex items-center justify-center text-center p-6">
                        <div>
                            ...
                            <h1 className="text-4xl font-bold text-gray-100 mb-2">No Case Selected</h1>
                            <p className="text-lg text-gray-400 mb-6">Please select an alert from the dashboard to begin an investigation.</p>
                            <Link to="/dashboard" className="bg-cyan-600 hover:bg-cyan-500 text-white font-bold py-3 px-6 rounded-lg transition">
                                Return to Dashboard
                            </Link>
                        </div>
                    </main>
                ...
                <main className="flex-1 overflow-x-hidden overflow-y-auto bg-gray-900/50 p-6 pt-24">
                    <div className="max-w-full mx-auto px-4">
                        <div className="mb-6 flex items-center justify-between animate-fadeIn">
                            ...
                            <button onClick={() => setIsGraphFullscreen(!isGraphFullscreen)} className="bg-gray-700/50 hover:bg-gray-600 p-2 rounded-lg transition" title="Toggle Graph Fullscreen">
                                {isGraphFullscreen ? <Minimize2 className="h-5 w-5" /> : <Maximize2 className="h-5 w-5" />}
                            </button>
                        </div>
                        
                        <div className={`flex flex-col xl:flex-row gap-6 ${isGraphFullscreen ? 'h-[calc(100vh-10rem)]' : 'h-[calc(100vh-12rem)]'}`}>
                            {!isGraphFullscreen && (
                                <div className="xl:w-1/3 flex flex-col gap-6 overflow-y-auto pr-2">
                                    ...
                                    <InfoPanel title="Subject Details" icon={User}>
                                        <div className="space-y-3 text-sm">
                                            <DetailRow label="Name" value={personDetails?.full_name} />
                                            <DetailRow label="PAN" value={personDetails?.pan_number} />
                                            <DetailRow label="Salary" value={personDetails?.monthly_salary_inr ? `₹${personDetails.monthly_salary_inr.toLocaleString('en-IN')} /mo` : 'N/A'} />
                                            <DetailRow label="Address" value={personDetails?.address} />
                                        </div>
                                    </InfoPanel>
                                    <InfoPanel title="Investigator Notes" icon={StickyNote}>
                                        <textarea className="w-full h-48 bg-gray-900/70 border border-gray-600 rounded-lg p-3 text-white placeholder-gray-500 focus:ring-2 focus:ring-purple-500 focus:outline-none transition" placeholder="Add your observations..."></textarea>
                                        <button className="mt-3 w-full bg-purple-600/50 hover:bg-purple-600 text-white font-semibold py-2 px-4 rounded-lg transition">Save Notes</button>
                                    </InfoPanel>
                                </div>
                            )}
                            <div className={`flex-1 h-full min-h-[500px] ${isGraphFullscreen ? 'xl:w-full' : 'xl:w-2/3'}`}>
...
const InfoPanel = ({ title, icon: Icon, children, fullHeight = false }) => (
    <div className={`bg-gray-800/50 rounded-xl border border-gray-700 ${fullHeight ? 'flex flex-col h-full' : ''}`}>
