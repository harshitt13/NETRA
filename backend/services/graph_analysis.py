...
#     def get_transaction_network(self, entity_id, limit=25):
#         if self._driver is None:
#             return {"nodes": [], "edges": []}
            
#         with self._driver.session() as session:
#             # --- THIS IS THE FIX ---
#             # This new query is more flexible. It finds an entity by its ID (person or company),
#             # then finds all transactions (both incoming and outgoing) associated with it.
#             query = """
#             MATCH (entity)
#             WHERE entity.person_id = $entity_id OR entity.cin = $entity_id
#             MATCH (entity)-[:OWNS_ACCOUNT]->(account)
...
#                 // Determine direction for the graph
#                 startNode(tx) = account AS isOutgoing
#             ORDER BY tx.timestamp DESC
#             LIMIT $limit
#             """
...
#                     center_node_id = source_id

#                 # Add source node (the entity being investigated)
#                 if source_id not in nodes:
#                     nodes[source_id] = {"id": source_id, "label": record["sourceName"], "isCenter": True}
                
#                 # Add target/peer node
#                 target_id = record["targetId"]
#                 if target_id not in nodes:
#                     nodes[target_id] = {"id": target_id, "label": record["targetName"], "isCenter": False}
                
#                 # Add the transaction edge, determining source and target based on direction
#                 is_outgoing = record["isOutgoing"]
...
#                     "source": source,
#                     "target": target,
#                     "label": str(record["amount"])
#                 })
            
#             # The frontend's NetworkGraph component expects a property called 'edges'
#             return {"nodes": list(nodes.values()), "edges": edges}
